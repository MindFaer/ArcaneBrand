...
public class ArcaneInfusionEarth extends ArcaneEnchantment {
    private static Random random = new Random();

    public ArcaneInfusionEarth(String id, int maxLevel) {
        super(id, maxLevel);
    }
        ...
        if (!attacker.level().isClientSide) {
            if (attacker instanceof Player player) {
                ...
                if (ArcaneEnchantmentUtils.isArcaneItem(stack)) {
                    int enchantmentLevel = ArcaneBrandEnchantmentUtils.getArcaneEnchantment(stack, ArcaneBrandMain.ARCANE_INFUSION_EARTH);
